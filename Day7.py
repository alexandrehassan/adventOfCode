...
class bagInfo:
    ...
    def add_contents(self, quantity: int, name: str) -> None:
        name = name.strip()
        if name not in self.contents:
            self.contents[name] = quantity
            if name == "shiny gold":
                self.gold_content = quantity

    def number_of_bags(self) -> int:
        return len(self.contents)
    
    def contains_gold(self) -> bool:
        return self.gold_content !=0
    
    def number_of_gold(self) -> int:
        return self.gold_content
...
def count_possible_bags() -> int:
    ...
    for bag_name in bags:
        if look_for_gold(bag_name, bags):
            count+=1

    return count
        
def look_for_gold(bag_name: str, bags: dict) -> bool:
    bag = bags[bag_name]
    if bag in cache:
        return cache[bag]
    elif bag.contains_gold():
        cache[bag] = True
        return True
    elif bag.number_of_bags == 0:
        cache[bag] = False
        return False
    else:
        for inner_name in bag.contents:
            if look_for_gold(inner_name, bags):
                return True
        return False
...
def parse_line(line: str) -> bagInfo:
    ...
    for inner in inner_bags:
        inner = inner.replace("bags", "").replace("bag", "").strip()
        inner_split = inner.split(" ", 1)
        if inner_split[0] != "no":
            bag.add_contents(int(inner_split[0]), inner_split[1])
    return bag
...
def count_bags(name: str, bags: list) -> int:
    if bags[name].number_of_bags() == 0:
        return 1
    in_bag = bags[name].contents
    count = 1
    for bag in in_bag:
