...
def find_first_wrong(lines: list, offset: int) -> int:
    ...
    for i in range(len(lines)-offset):
        if not check_valid(lines[i:i+offset], lines[i+offset]):
            return lines[i+offset]

def check_valid(lines: list, num: str) -> bool:
    for first in lines:
        if num - first in lines and num - first != num // 2:
            return True
    return False

def find_weakness(lines: list) -> int:
    invalid = find_first_wrong(lines, 25)
    index_of_invalid = lines.index(invalid)
    return look_for_weakness(invalid, lines, index_of_invalid-2, index_of_invalid-1)

def look_for_weakness(sum_to: int, lines: list, start: int, end: int)-> int:
    sum_of = sum(lines[start:end])
    if sum_of == sum_to:
        return max(lines[start:end]) + min(lines[start:end])
    elif sum_of < sum_to:
        return look_for_weakness(sum_to, lines, start-1, end)
    else: 
        return look_for_weakness(sum_to, lines, end-2, end-1)
